...
class UpworkEvaluator {
  ...
  evaluate(job, settings) {
    ...
    // Filter 1b — Fixed-price budget floor
    const budgetFloor = parseFloat(settings.min_fixed_budget || '200');
    if (job.jobType === 'fixed' && job.budget !== null && job.budget < budgetFloor) {
      return { status: 'filtered', reason: `budget_below_floor:${job.budget}<${budgetFloor}` };
    }
  ...
  static scoreJob(job) {
    ...
    const bradSkills = ['python', 'javascript', 'playwright', 'automation', 'ai', 'claude',
      ...
      'node', 'nodejs', 'typescript', 'api', 'web', 'chatgpt', 'openai', 'gpt',
      'browser', 'selenium', 'puppeteer', 'make', 'zapier'];
    const jobText = `${job.title || ''} ${job.description || ''} ${job.skills || ''}`.toLowerCase();
    const matchedSkills = bradSkills.filter(s => new RegExp(`\\b${s}\\b`, 'i').test(jobText));
    ...
    if (job.clientTotalSpent) {
      const spentText = job.clientTotalSpent.toLowerCase();
      if (/1m\+/i.test(spentText) || /\$1,000,000/i.test(spentText)) {
        clientScore += 5;
      } else {
        // Extract numeric value — handles "$100K+", "$50k+", "$10K+", "$5,000"
        const kMatch = spentText.match(/([\d,.]+)\s*k/i);
        const rawMatch = spentText.match(/\$?([\d,]+)/);
        let spentK = 0;
        if (kMatch) {
          spentK = parseFloat(kMatch[1].replace(/,/g, ''));
        } else if (rawMatch) {
          spentK = parseFloat(rawMatch[1].replace(/,/g, '')) / 1000;
        }
        if (spentK >= 100) clientScore += 5;
        else if (spentK >= 10) clientScore += 3;
        else if (spentK >= 1) clientScore += 1;
      }
    }
    // Penalize unvetted clients: no spend history AND no rating = risky
    if (!job.clientTotalSpent && job.clientRating == null && !job.clientPaymentVerified) {
      clientScore = Math.max(0, clientScore - 5);
    }
