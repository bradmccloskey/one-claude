...
class NotificationManager {
   ...
   */
  // Map string tier names (from AI brain) to numeric constants
  static TIER_MAP = {
    urgent: 1, critical: 1, emergency: 1,
    action: 2, important: 2, high: 2,
    summary: 3, info: 3, low: 3, sms: 3, batch: 3,
    debug: 4, silent: 4, log: 4,
  };

  notify(text, tier = NotificationManager.ACTION) {
    // Normalize string tiers to numeric
    if (typeof tier === 'string') {
      tier = NotificationManager.TIER_MAP[tier.toLowerCase()] || NotificationManager.ACTION;
    }
  ...
  _flushBatch() {
    ...
    // Clean each item: collapse to single line, cap length
    const cleanItems = items.map((item) => {
      return item
        .replace(/\n/g, " ")     // collapse newlines to spaces
        .replace(/\s{2,}/g, " ") // collapse whitespace
        .trim()
        .substring(0, 200);      // cap each item
    });
