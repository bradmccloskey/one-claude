...
class SignalProtocol {
  ...
  formatSignalNotification(signal) {
    ...
    switch (signal.type) {
      case "needs-input":
        lines.push(`INPUT NEEDED: ${signal.projectName}`);
        lines.push("");
        lines.push(signal.data.question || "No question specified");
        if (signal.data.context) {
          lines.push("");
          lines.push(`Context: ${signal.data.context}`);
        }
        ...
        lines.push("");
        lines.push(`Reply: reply ${signal.projectName}: <your answer>`);
        break;

      case "completed":
        lines.push(`COMPLETED: ${signal.projectName}`);
        lines.push("");
      ...
      case "error":
        lines.push(`ERROR: ${signal.projectName}`);
        lines.push("");
        lines.push(signal.data.error || "Unknown error");
        if (signal.data.context) {
          lines.push(`While: ${signal.data.context}`);
        }
        break;
    }
