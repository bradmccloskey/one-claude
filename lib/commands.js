...
class CommandRouter {
  ...
  _handleNaturalLanguage(text) {
    try {
      ...
      const context = this.aiBrain.contextAssembler.assemble();
      ...
      const prompt = [
        'You are Brad\'s AI project orchestrator. You manage ~18 projects with Claude Code sessions on a Mac Mini.',
        'Respond conversationally via SMS. Keep it under 1000 chars. Be direct and useful.',
        'You are the primary interface — Brad talks to you naturally, not with menu commands.',
        '',
      ...
      if (response.length < 5) return this._handleHelp();

      // Truncate to SMS limit
      return response.length > 1500
        ? response.substring(0, 1460) + '\n\n[truncated]'
        : response;

    } catch (err) {
      console.log(`[commands] Natural language failed: ${err.message}`);
      return this._handleHelp();
    }
  }
