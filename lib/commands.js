...
class CommandRouter {
  ...
  /**
   * Route a command. Accepts optional onProgress callback for streaming.
   * @param {string} text - Command text
   * @param {Object} [opts] - Options
   * @param {Function} [opts.onProgress] - Callback for streaming NL output chunks
  ...
  async route(text, opts = {}) {
    ...
    // Action commands — intercept these when they match a known project.
    // If the text after "stop/start/restart" doesn't match a project,
    // fall through to the AI natural language handler instead of erroring.
    if (lower.startsWith("start ")) {
      const result = this._tryHandleStart(trimmed.substring(6).trim());
      if (result) return result;
    }
    if (lower === "startall" || lower === "start all") return this._handleStartAll();
    if (lower.startsWith("stop ")) {
      const result = this._tryHandleStop(trimmed.substring(5).trim());
      if (result) return result;
    }
    ...
    if (lower.startsWith("restart ")) {
      const result = this._tryHandleRestart(trimmed.substring(8).trim());
      if (result) return result;
    }
    ...
    if (this.aiBrain && this.aiBrain.isEnabled()) {
      return await this._handleNaturalLanguage(text, opts.onProgress);
    }
  ...
  // _tryHandle variants return null when no project matches, allowing fallthrough to AI NL
  _tryHandleStart(input) {
    const match = this._matchProjectName(input);
    if (match) return this._handleStart(input);
    // Check if there's context or an embedded project ref — if so, still handle
    if (this._getContext()) return this._handleStart(input);
    return null; // no match — let AI handle it
  }

  _tryHandleStop(name) {
    const match = this._matchProjectName(name);
    if (match) return this._handleStop(name);
    if (this._getContext()) return this._handleStop(name);
    return null; // no match — let AI handle it
  }

  _tryHandleRestart(name) {
    const match = this._matchProjectName(name);
    if (match) return this._handleRestart(name);
    if (this._getContext()) return this._handleRestart(name);
    return null; // no match — let AI handle it
  }
  ...
  /**
   * Assemble rich NL context with all project data pre-computed.
   * Claude can answer "status of all projects" in 1 turn, zero tool calls.
  ...
  _assembleNLContext() {
    ...
    // System overview
    sections.push(`Autonomy: ${autonomyLevel}`);
    sections.push(`Time: ${new Date().toISOString()}`);
    sections.push(`Active sessions (${sessions.length}): ${sessions.length > 0 ? sessions.map(ss => {
      const dur = Math.round((Date.now() - new Date(ss.created).getTime()) / 60000);
      return `${ss.projectName} (${dur}min)`;
    }).join(', ') : 'none'}`);

    // Resource snapshot
    if (this.aiBrain.contextAssembler?.resourceMonitor) {
      try {
        const snap = this.aiBrain.contextAssembler.resourceMonitor.getSnapshot();
        sections.push(`System: CPU ${snap.cpuLoadAvg1m?.toFixed(1) || '?'}/${snap.cpuCount || '?'} cores | RAM ${snap.memUsedPct || '?'}% used | Disk ${snap.diskUsedPct || '?'}%`);
      } catch {}
    }

    // Health status
    if (this.aiBrain.contextAssembler?.healthMonitor) {
      try {
        const stats = this.aiBrain.contextAssembler.healthMonitor.getStats();
        const results = this.aiBrain.contextAssembler.healthMonitor.getLastResults();
        const downServices = Object.values(results || {}).filter(r => r.status === 'down').map(r => r.name);
        sections.push(`Services: ${stats.up || 0}/${stats.total || 0} up${downServices.length > 0 ? ' | DOWN: ' + downServices.join(', ') : ''}`);
      } catch {}
    }

    // All projects with full detail
    sections.push('');
    sections.push(`--- All Projects (${projects.length}) ---`);
    for (const p of projects) {
      if (!p.exists) continue;
      const parts = [];
      const running = sessionSet.has(p.name) ? ' [RUNNING]' : '';
      parts.push(`${p.name}${running}`);

      const statusParts = [];
      if (p.status) statusParts.push(p.status);
      if (p.phase != null && p.totalPhases != null) statusParts.push(`phase ${p.phase}/${p.totalPhases}`);
      if (p.progress != null) statusParts.push(`${p.progress}%`);
      if (statusParts.length > 0) parts.push(`  ${statusParts.join(' | ')}`);

      if (p.currentFocus) parts.push(`  Focus: ${p.currentFocus}`);
      if (p.blockers?.length > 0) parts.push(`  Blockers: ${p.blockers.join('; ')}`);
      if (p.nextSteps?.length > 0) parts.push(`  Next: ${p.nextSteps.slice(0, 2).join('; ')}`);
      if (p.needsAttention) parts.push(`  ATTENTION: ${p.attentionReason}`);
      if (p.lastActivity) parts.push(`  Last: ${p.lastActivity}`);
  ...
  /**
   * Parse ACTION_JSON tags from AI response and execute them.
   * Returns { cleanedResponse, actionResults }.
   * @param {string} response - Raw AI response
   * @returns {{ cleanedResponse: string, actionResults: string[] }}
   */
  _executeActionIntents(response) {
    const actionResults = [];
    const actionRegex = /ACTION_JSON:\s*(\{[^}]+\})/g;
    let match;
    ...
    while ((match = actionRegex.exec(response)) !== null) {
      try {
        ...
        console.log(`[commands] Failed to parse ACTION_JSON: ${e.message}`);
      }
    }

    // Strip ACTION_JSON lines from response
    const cleanedResponse = response.replace(/ACTION_JSON:\s*\{[^}]+\}/g, '').trim();
    return { cleanedResponse, actionResults };
  }

  /**
   * Execute a single action intent.
   * @param {Object} action - { action, project, prompt?, level? }
   * @returns {string|null} Result message or null
   */
  _executeAction(action) {
    const { action: act, project, prompt, level } = action;

    switch (act) {
      case 'start': {
        const match = this._matchProjectName(project);
        if (!match) return `Could not find project "${project}"`;
        this.signalProtocol.injectClaudeMd(match);
        const result = this.sessionManager.startSession(match, prompt);
        return `Started ${match}: ${result.message}`;
      }
      case 'stop': {
        const match = this._matchProjectName(project);
        if (!match) return `Could not find project "${project}"`;
        const result = this.sessionManager.stopSession(match);
        return `Stopped ${match}: ${result.message}`;
      }
      case 'restart': {
        const match = this._matchProjectName(project);
        if (!match) return `Could not find project "${project}"`;
        this.signalProtocol.injectClaudeMd(match);
        const result = this.sessionManager.restartSession(match);
        return `Restarted ${match}: ${result.message}`;
      }
      case 'set_level': {
        const validLevels = ['observe', 'cautious', 'moderate', 'full'];
        const lvl = (level || '').toLowerCase();
        if (!validLevels.includes(lvl)) return `Invalid level "${level}"`;
        const s = this.state.load();
        this.state.setAutonomyLevel(s, lvl);
        if (this.trustTracker) this.trustTracker.resetPromotionFlag();
        return `Autonomy set to ${lvl}`;
      }
      default:
        console.log(`[commands] Unknown action intent: ${act}`);
        return null;
    }
  }
   ...
   * Uses rich pre-computed context so Claude can answer in 1 turn for most queries.
   * Supports ACTION_JSON tags for session management and streaming output.
   *
   * @param {string} text - User message
   * @param {Function} [onProgress] - Optional callback for streaming stdout chunks
   */
  async _handleNaturalLanguage(text, onProgress) {
    try {
      // Pre-assemble rich context (all projects, sessions, health, resources)
      const nlContext = this._assembleNLContext();
      ...
      // Read NL config
      const nlConfig = this.aiBrain.config.ai?.nl || {};
      const maxTurns = nlConfig.maxTurns || 25;
      const timeout = nlConfig.timeout || 300000;
      ...
      const prompt = [
        ...
        '',
        '--- ACTION CAPABILITIES ---',
        'You can take actions by including ACTION_JSON tags in your response (one per line, at the end):',
        'ACTION_JSON:{"action":"start","project":"land-speculation","prompt":"Continue phase 3 work"}',
        'ACTION_JSON:{"action":"stop","project":"crypto-trader"}',
        'ACTION_JSON:{"action":"restart","project":"youtube-automation"}',
        'ACTION_JSON:{"action":"set_level","level":"moderate"}',
        'Multiple ACTION_JSON lines are supported for batch operations.',
        'Only use ACTION_JSON when the user explicitly asks to start/stop/restart a session or change autonomy level.',
        '',
        '--- REMINDERS ---',
        'If the user wants to set a reminder, include on its own line: REMINDER_JSON:{"text":"what to remind about","fireAt":"ISO-8601-timestamp"}',
        'The fireAt must be an absolute ISO 8601 timestamp in America/New_York timezone. Current time: ' + new Date().toISOString(),
        '',
        ...
        'HOWEVER: The full project state is already provided below. For "status" or "overview" questions, answer directly from the context — no tool calls needed.',
        '',
      ...
      if (response.startsWith('Error:') || response.includes('Reached max turns')) {
        ...
        const sessionList = activeSessions.length > 0
          ? `Active sessions: ${activeSessions.map(ss => ss.projectName).join(', ')}`
          : 'No active sessions.';
      ...
      // Execute ACTION_JSON intents
      const { cleanedResponse, actionResults } = this._executeActionIntents(response);

      // Detect and handle reminder intent
      let reminderConfirmation = '';
      if (this.reminderManager) {
        const reminderMatch = cleanedResponse.match(/REMINDER_JSON:\s*(\{[^}]+\})/);
        if (reminderMatch) {
          try {
      ...
      if (cleaned.length < 5) cleaned = cleanedResponse.trim();
