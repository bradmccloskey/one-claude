...
class CommandRouter {
  ...
  _handleNaturalLanguage(text) {
    try {
      ...
      const recentHistory = this._conversationHistory.slice(-4)
        .map(h => `${h.role}: ${h.text}`).join('\n');
      ...
      this._conversationHistory.push({ role: 'user', text, ts: Date.now() });

      const model = this.aiBrain.config.ai?.model || 'sonnet';
      const response = execSync(
        `claude -p --model ${model} --output-format text --dangerously-skip-permissions`,
      ...
      if (response.startsWith('Error:') || response.includes('Reached max turns')) {
        ...
        return 'That took too long to look up. Try a more specific question, or I can start a session on that project.';
      }

      if (response.length < 5) return 'Sorry, I couldn\'t process that. Try rephrasing?';
      ...
      const finalResponse = cleaned.length > 1500
        ? cleaned.substring(0, 1460) + '\n\n[truncated]'
        : cleaned;
      this._conversationHistory.push({ role: 'assistant', text: finalResponse.substring(0, 200), ts: Date.now() });
      ...
      if (errMsg.includes('max turns') || errMsg.includes('Reached max')) {
        return 'That question needed more time than I had. Try asking something more specific, or I can start a Claude session on that project to dig deeper.';
      }
      return 'Sorry, hit an error processing that. Try again or text "ai off" for legacy mode.';
    }
  }
