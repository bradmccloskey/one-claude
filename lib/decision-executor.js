...
class DecisionExecutor {
  ...
  async execute(evaluatedRecommendation) {
    ...
    if (!this._isActionAllowed(rec.action, autonomyLevel)) {
      // Check dedup before notifying — prevents repeated "AI would restart X" spam
      if (this._isDuplicate(rec)) {
        return { executed: false, action: rec.action, project: rec.project, rejected: "duplicate_suppressed", autonomyLevel };
      }
      this._recordRecommendation(rec);
      const shortReason = rec.reason ? rec.reason.split(/\.\s/)[0].substring(0, 100) : "";
      const smsText = `AI would ${rec.action} ${rec.project}${shortReason ? ": " + shortReason : ""}`;
    ...
    if (result.success && ["start", "stop", "restart"].includes(rec.action)) {
      // Keep reason concise for SMS — first sentence, max 100 chars
      const shortReason = rec.reason
        ? rec.reason.split(/\.\s/)[0].substring(0, 100)
        : "";
      const actionText = `AI ${rec.action}ed ${rec.project}${shortReason ? ": " + shortReason : ""}`;
