...
class StateManager {
  ...
  // --- Phase 5: Health Restart History ---

  /**
   * Log a health restart event to the restart history.
   * Trims to last 100 entries to prevent unbounded growth.
   * @param {Object} state - Current state object
   * @param {Object} record - Restart record
   * @param {string} record.service - Service name
   * @param {string} record.type - Restart type (launchd, docker)
   * @param {string} record.timestamp - ISO timestamp
   * @param {boolean} record.success - Whether the restart succeeded
   */
  logHealthRestart(state, record) {
    if (!state.healthRestartHistory) state.healthRestartHistory = [];
    state.healthRestartHistory.push(record);
    if (state.healthRestartHistory.length > 100) {
      state.healthRestartHistory = state.healthRestartHistory.slice(-100);
    }
    this.save(state);
  }

  /**
   * Get the most recent health restart events.
   * @param {Object} state - Current state object
   * @param {number} [count=10] - Number of recent restarts to return
   * @returns {Array} Last N restart entries
   */
