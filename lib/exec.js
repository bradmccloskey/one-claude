...
function claudeP(prompt, options = {}) {
  ...
  // Only used for NL handler where tools are already restricted via allowedTools
  ...
  if (jsonSchema) {
    // Use text output format — --json-schema constrains model output to valid JSON,
    // but --output-format json wraps it in a CLI result envelope that breaks parsing.
  ...
  if (allowedTools) {
    if (allowedTools.length === 0) {
      // Empty array = no tools allowed (forces model to respond immediately)
      parts.push('--allowedTools', '""');
    } else {
      for (const tool of allowedTools) {
        // Quote tool names containing shell metacharacters like Bash(pattern:*)
        const escaped = tool.includes('(') ? `'${tool}'` : tool;
        parts.push('--allowedTools', escaped);
      }
    }
  }
