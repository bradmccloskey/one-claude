...
class SessionManager {
  ...
  startSession(projectName, prompt) {
    ...
    const sessionName = this.sessionPrefix + projectName;
    ...
    try {
      execSync(
        `tmux new-session -d -s "${sessionName}" -c "${projectDir}" ` +
          `'claude --dangerously-skip-permissions -p '"'"'${escapedPrompt}'"'"' 2>&1 | tee .orchestrator/session.log; ` +
          `echo "__ORCH_SESSION_ENDED__" >> .orchestrator/session.log'`,
        { timeout: 10000 }
      );
  ...
  sendInput(projectName, input) {
    ...
    try {
      ...
      execSync(
        `tmux load-buffer "${tmpFile}" \\; paste-buffer -t "${sessionName}" \\; ` +
    ...
    try {
      ...
      return output
        ...
        .map((line) => {
          ...
          return {
            name,
            projectName: name.substring(this.sessionPrefix.length),
  ...
  _buildResumePrompt(projectName, projectDir) {
    ...
    if (fs.existsSync(stateFile)) {
      return (
        "Resume work on this project. Check .planning/STATE.md for current status and continue from where we left off. " +
        "Work autonomously. When you complete a significant milestone or encounter a blocker that requires human input, " +
        "write a JSON file to .orchestrator/needs-input.json with {\"question\": \"your question\", \"context\": \"brief context\"} " +
        "and then stop and wait. When you finish all planned work, write to .orchestrator/completed.json with " +
        "{\"summary\": \"what was accomplished\"}. If you hit an unrecoverable error, write to .orchestrator/error.json " +
        "with {\"error\": \"description\", \"context\": \"what you were doing\"}."
      );
    }
    return (
      `Review the ${projectName} project and determine what needs to be done. ` +
      "If there is a CLAUDE.md or README, follow those instructions. Work autonomously. " +
      "When you need human input, write to .orchestrator/needs-input.json with your question."
    );
  }
