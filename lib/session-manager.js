...
class SessionManager {
  ...
  /**
   * Build a launcher script for tmux sessions.
   *
   * Always created (not just for keychain unlock) so that the script can
   * explicitly `cd` into the project directory.  macOS tmux sessions spawned
   * via launchd can ignore the `-c` start-directory flag, inheriting the
   * tmux server's cwd (the orchestrator directory) instead.  An explicit
   * `cd` in the launcher guarantees Claude Code starts in the right place.
   *
   * If a .keychain-password file exists the script also unlocks the macOS
   * keychain before starting Claude (required because launchd-spawned tmux
   * sessions don't inherit the security session).
   *
   * @param {string} signalDir - Path to write the launcher script
   * @param {string} mcpConfigFlag - Optional --mcp-config flag
   * @param {string} projectDir - Absolute path to the project directory
   * @returns {string} Path to the launcher script
   */
  _buildLauncher(signalDir, mcpConfigFlag, projectDir) {
    ...
    // Optionally unlock keychain if password file exists
    const pwFile = path.join(__dirname, '..', '.keychain-password');
    if (fs.existsSync(pwFile)) {
      const password = fs.readFileSync(pwFile, 'utf-8').trim();
      lines.push(`security unlock-keychain -p "${password}" ~/Library/Keychains/login.keychain-db 2>/dev/null`);
    }

    lines.push(`exec claude --dangerously-skip-permissions${mcpConfigFlag}`);
  ...
  startSession(projectName, prompt, options = {}) {
    ...
    try {
      // Build launcher script that cd's into the project directory (and
      // optionally unlocks the macOS keychain) before starting Claude.
      // The explicit cd is critical: macOS tmux sessions spawned via launchd
      // can ignore the -c start-directory flag, falling back to the tmux
      // server's cwd (the orchestrator directory) instead.
      const launcherPath = this._buildLauncher(signalDir, mcpConfigFlag, projectDir);

      // Launch tmux with claude in interactive mode
      const tmuxCmd = `tmux new-session -d -s "${sessionName}" -c "${projectDir}" "${launcherPath}"`;
