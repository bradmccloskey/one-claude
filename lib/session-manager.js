...
class SessionManager {
  ...
  _buildResumePrompt(projectName, projectDir) {
    // Check for previous session evaluation and original prompt (SESS-03)
    let evalContext = '';
    let originalPrompt = '';
    try {
      ...
      if (fs.existsSync(sessionFile)) {
        ...
        if (sessionData.fullPrompt) {
          originalPrompt = sessionData.fullPrompt;
        }
      }
    } catch {} // Session file may be missing or malformed
    ...
    try {
      ...
      if (fs.existsSync(evalFile)) {
        ...
        // If we have the original prompt and this is a resume after timeout,
        // include the original task so the new session knows what to work on
        if (originalPrompt && (evalData.recommendation === 'continue' || evalData.recommendation === 'retry')) {
          evalContext += `Original task prompt:\n${originalPrompt}\n\n`;
        }
      }
