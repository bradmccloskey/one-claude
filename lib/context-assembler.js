...
class ContextAssembler {
   ...
   */
  constructor({ scanner, sessionManager, processMonitor, state, config, resourceMonitor, healthMonitor, mcpBridge, revenueTracker, trustTracker, conversationStore, sessionLearner }) {
  ...
  /**
   * Build conversation memory section for AI context.
   * Shows recent exchanges as previews with age.
   * @returns {string|null}
   */
  _buildConversationSection() {
    if (!this.conversationStore) return null;
    try {
      const entries = this.conversationStore.getAll();
      if (!entries || entries.length === 0) return null;

      const now = Date.now();
      // Show last 6 entries as previews
      const recent = entries.slice(-6);

      const lines = [`Conversation Memory (${entries.length} exchanges, 7-day window):`];
      for (const entry of recent) {
        const ageMs = now - entry.ts;
        const ageStr = this._formatConversationAge(ageMs);
  ...
  /**
   * Format a conversation entry age into a human-readable string.
   * @param {number} ageMs - Age in milliseconds
   * @returns {string}
   * @private
   */
  _formatConversationAge(ageMs) {
  ...
  /**
   * Build session learning patterns section for AI context.
  ...
  _buildResponseFormat() {
    ...
    const lines = [
      "IMPORTANT: Respond with ONLY a JSON object. No prose, no explanation, no markdown fences. Just raw JSON.",
      "",
      "Required JSON structure:",
      '{"recommendations": [...], "summary": "...", "nextThinkIn": 300}',
      "",
      "Each recommendation: {project, action, reason, priority, confidence, prompt?, message?, notificationTier?}",
      "- action: start | stop | restart | notify | skip",
      "- priority: 1=critical ... 5=informational",
      "- nextThinkIn: seconds until next cycle (60-1800)",
      "",
