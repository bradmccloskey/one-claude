...
- Shell out to claude using `child_process.execSync` with the `input` option to pass the prompt via stdin (no temp files):
  ```javascript
  ...
  ```
  - Use `input` option (not temp files or cat pipes) -- this is the clean, single approach
  - Timeout is 30000ms (30s) to match the phase success criteria of sub-30s think cycles
  - Wrap in try/catch handling three distinct error cases:
    1. Timeout error (e.code === 'ETIMEDOUT'): log "think timed out after 30s", return fallback decision
    2. Non-zero exit code (e.status !== 0): log stderr, return fallback decision
    3. Other exec errors: log error message, return fallback decision
...
`parseJSON(text)`:
- Public method (not underscore-prefixed) so it can be tested directly
...
3. ai-brain.js parseJSON (public method) handles bare JSON, markdown fenced JSON, and JSON embedded in prose
