...
<task type="auto">
  <name>Task 2a: Initialize AI modules and pass to CommandRouter in index.js</name>
  <files>index.js</files>
  <action>
Modify index.js to import, initialize, and wire the AI brain modules into the CommandRouter. This task handles imports and initialization only -- the think cycle, banner, and shutdown changes are in Task 2b.
...
- Verify the structural changes exist:
```bash
cd /Users/claude/projects/project-orchestrator && node -e "
const src = require('fs').readFileSync('index.js', 'utf-8');
...
console.log('Has aiBrain in CommandRouter:', src.includes('aiBrain'));
"
```
All should print true.
  </verify>
  <done>index.js imports all three AI modules, initializes them with correct dependencies, and passes aiBrain + decisionExecutor + messenger to CommandRouter</done>
</task>

<task type="auto">
  <name>Task 2b: Add think cycle timer, update banner and shutdown in index.js</name>
  <files>index.js</files>
  <action>
Continue modifying index.js to add the periodic think cycle, update the startup banner, and clean up on shutdown.

**1. Add AI think cycle** (after existing polling loops):
```javascript
// ── AI Think Cycle ────────────────────────────────────────────────────────
...
**2. Update startup banner** to show AI status:
...
**3. Update graceful shutdown** to clear the think interval:
Add before `process.exit(0)`:
```javascript
if (thinkInterval) clearInterval(thinkInterval);
```

**4. Update the version string** in the banner from "v2.0" to "v3.0".
  </action>
  <verify>
- Verify index.js can be parsed without syntax errors:
```bash
...
console.log('Has clearInterval:', src.includes('clearInterval(thinkInterval)'));
console.log('Has startThinkCycle:', src.includes('startThinkCycle'));
"
```
All should print true.
  </verify>
  <done>index.js has periodic think cycle on thinkIntervalMs timer (respects quiet hours + enabled check), shows AI status in startup banner, version bumped to v3.0, graceful shutdown clears think interval</done>
</task>
